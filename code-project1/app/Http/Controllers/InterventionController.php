...
class InterventionController extends Controller
{
    ...
    public function store(Request $request)
    {
        $validated = $request->validate([
            ...
            'id_technicien' => 'nullable|exists:techniciens,id',
            'description'   => 'required|string',
            'statut'        => 'nullable|string',
        ]);

        $validated['id_technicien'] = $validated['id_technicien'] ?? 1;
        $validated['statut'] = $validated['statut'] ?? 'en_attente';

        $intervention = Intervention::create($validated);

        return response()->json(['message' => 'Intervention created', 'data' => $intervention], 201);
    }
